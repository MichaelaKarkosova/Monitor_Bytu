...
class UlovDomovReader implements ChainableReaderInterface
{
    private database $db;
    ...
    {
        //čteme data z ulovdomovu
        $i = 1;
        $finalapartments = [];
        do {
            //sleep nám pomáhá získat data tak, aby nedošlo k timeoutu ze strany ulovdomovu
            //vytáhneme obsah webu
            $html = file_get_contents($source);
            $crawler = new Crawler($html);
            print_r($html);
            print_r($crawler->filter(".chakra-stack.css-1bia7oe.css-1bia7oe")->count());
            ...
            $source = NULL !== $source ? 'https://reality.ulovdomov.cz' . $source : NULL;
            $i++;
        } while ($source !== NULL);
        //vrátíme výsledek čtení - pole a informaci, z jakého zdroje se četlo.
        return new ApartmentsResult('ulovdomov', $finalapartments);
    }
    ...
    public function getDetails(): array
    {
        ...
        //vytáhneme všechny ulovdomov adresy na detaily z databáze
        $allapartments = $db->query("select * from byty where url like '%ulovdomov%'");
        foreach ($allapartments as $a) {
            ...
            //Na ulovdomovu se uvádí dispozice bytu v nadpise. Nadpis tedy rozdělíme na jednotlivé informace.
