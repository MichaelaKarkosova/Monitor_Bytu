...
class JobsCZReader implements ChainableReaderInterface {
    ...
    public function read(string $source): JobsResult {
                ...
                $apartments = $crawler->filter('.advert-list-items__items li')
                    ->each(static function (Crawler $item) {
                        ...
                        //upravíme part z formátu Praha x, část na Praha x - část.
                         ...
                         //vrátíme object Job
                         $a = new Job($href, $name, $href, 0, $finalprice, $finalpart, $longpart);
                         return $a;
                    });
                $i++;
        ...
        return new JobsResult('realitymix', array_filter($finalapartments));
    }
    ...
    public function getDetails(): array {
        ...
        foreach ($allapartments as $a) {
            ...
            $apartments_all[] = $crawler->filter('.advert-layout__information-wrapper')
                ->each(static function (Crawler $item) use ($url): JobsResult {
                    //uložíme si poznámku
                    ...
                    $ap_d =  new JobsResult($url , $alldata["animals"] ?? NULL, $alldata["furniture"] ?? NULL, $alldata["elevator"] ?? 0, $alldata["stairs"] ?? NULL, $alldata["condition"] ?? NULL, $alldata["size"] ?? NULL, $alldata["balcony"] ?? 0, $alldata["area"] ?? NULL);
                    return $ap_d;
                });
        }
